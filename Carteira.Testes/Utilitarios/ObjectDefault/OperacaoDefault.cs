...
namespace Carteira.Testes.Utilitarios.ObjectDefault
{
    public class OperacaoDefault
    {
        ...
        private void BuildOperacao()
        {
            ...
            // AGOSTO 2019
            var One = new Operacao(FundoImobiliarios.Where(fii => fii.Sigla == "BCIA11").First(), XP, 1M, 128.05M, TipoOperacao.Compra, new DateTime(2019, 08, 05), 0.03M);

            // OUTUBRO 2019
            var Two = new Operacao(FundoImobiliarios.Where(fii => fii.Sigla == "FEXC11").First(), XP, 1M, 112.64M, TipoOperacao.Compra, new DateTime(2019, 10, 31), 0.03M);
            ...
            var Seven = new Operacao(FundoImobiliarios.Where(fii => fii.Sigla == "HSML11").First(), XP, 1M, 123.50m, TipoOperacao.Compra, new DateTime(2020, 01, 07), 0.03M);
            ...
            // FEVEREIRO 2020
            var Twelve = new Operacao(FundoImobiliarios.Where(fii => fii.Sigla == "VISC11").First(), XP, 1M, 131M, TipoOperacao.Compra, new DateTime(2020, 02, 12), 0.03M);
            ...
            var Sixteen = new Operacao(FundoImobiliarios.Where(fii => fii.Sigla == "BCFF11").First(), XP, 2M, 96.80M, TipoOperacao.Compra, new DateTime(2020, 03, 13), 0.05M);
            var Seventeen = new Operacao(FundoImobiliarios.Where(fii => fii.Sigla == "FEXC11").First(), XP, 1M, 108.85M, TipoOperacao.Compra, new DateTime(2020, 03, 13), 0.03M);
            ...
            var Twenty = new Operacao(FundoImobiliarios.Where(fii => fii.Sigla == "BCIA11").First(), CLEAR, 1M, 112M, TipoOperacao.Compra, new DateTime(2020, 03, 19), 0.03M);
            ...
            var TwentyThree = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "BCIA11").First(), CLEAR, 1M, 178.00M, TipoOperacao.Venda, new DateTime(2020, 04, 23), 0.04M);
            var TwentyFour = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "HSML11").First(), XP, 1M, 84.06M, TipoOperacao.Venda, new DateTime(2020, 04, 23), 0.02M);
            var TwentyFive = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "VISC11").First(), XP, 1M, 100.30M, TipoOperacao.Venda, new DateTime(2020, 04, 23), 0.03M);
            var TwentySix = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "IRDM11").First(), XP, 2M, 102.00M, TipoOperacao.Compra, new DateTime(2020, 04, 23), 0.06M);
            
            // MAIO 2020
            var TwentySeven = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "HSML11").First(), XP, 2M, 80.00M, TipoOperacao.Compra, new DateTime(2020, 05, 12), 0.05M);
            var TwentyEight = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "VISC11").First(), XP, 1M, 96.60M, TipoOperacao.Compra, new DateTime(2020, 05, 12), 0.02M);
            var TwentyNine = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "VISC11").First(), XP, 1M, 95.85M, TipoOperacao.Compra, new DateTime(2020, 05, 12), 0.02M);

            var Trinta = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "BTLG11").First(), XP, 1M, 104.58M, TipoOperacao.Compra, new DateTime(2020, 07, 08), 0.02M);
            var TrintaEUm = new Operacao(FundoImobiliarios.Where(ts => ts.Sigla == "VINO11").First(), XP, 1M, 57.00M, TipoOperacao.Compra, new DateTime(2020, 07, 08), 0.01M);
            ...
            _operacoes.AddRange(new List<Operacao>() { One, Two, Three, Four, Five, Six, Seven, Eight, Nine, Ten,
                                                       ...
                                                       TwentyOne, TwentyTwo, TwentyThree, TwentyFour, TwentyFive, TwentySix, TwentySeven, TwentyEight, TwentyNine, Trinta, TrintaEUm});
        }

        private void BuildFundosImobiliarios()
        {
